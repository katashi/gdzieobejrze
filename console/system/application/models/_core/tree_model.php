...
class Tree_Model extends Model {
	...
	function tree_create_children($parent, $level, $table) {
		...
		if ($table == 'image') {
			...
			FROM
			`image_relations`
			Left Join `image` ON `image`.`id` = `image_relations`.`id_element`
			WHERE
			`image_relations`.`id_tree` =  '".$id_tree."'
			ORDER BY
			`image_relations`.`position`");
			foreach ($query->result() as $item) {
				$record = array();
				$record['id_relations'] = $item->mri;
				$record['tree'] = $table;
		...
		if ($table == 'article') {
			$query = $this->db->query("SELECT
			`article_relations`.`id` as `mri`,
			`article`.`id`,
			`article`.`title`
			FROM
			`article_relations`
			Left Join `article` ON `article`.`id` = `article_relations`.`id_element`
			WHERE
			`article_relations`.`id_tree` =  '".$id_tree."'
			ORDER BY
			`article_relations`.`position`");
			foreach ($query->result() as $item) {
				$record = array();
				$record['id_relations'] = $item->mri;
				$record['tree'] = $table;
